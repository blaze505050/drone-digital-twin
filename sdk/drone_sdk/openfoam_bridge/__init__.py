...
class AeroDatabase:
    ...
    def to_training_data(self) -> object:
        """Convert OpenFOAM results directly into pinn_engine.TrainingData.

        Feeds inputs=[velocity_ms, aoa_rad, Re] and outputs=[CL, CD] into
        PINNFactory.aerodynamics_pinn().fit(), connecting CFD to the neural surrogate.
        """
        from drone_sdk.pinn_engine import TrainingData
        arr = self.to_numpy()
        if len(arr) == 0:
            # Fallback or synthetic representative sample if empty
            u = np.array([5.0, 10.0, 15.0, 20.0])
            aoa = np.radians(np.array([0.0, 4.0, 8.0, 12.0]))
            re = u * self._lref / 1.48e-5
            x = np.column_stack([u, aoa, re])
            cl = 2.0 * math.pi * aoa
            cd = 0.02 + 0.05 * (cl ** 2)
        ...
        vel = arr[:, 0]
        aoa_rad = np.radians(arr[:, 1])
        re = vel * self._lref / 1.48e-5
        x = np.column_stack([vel, aoa_rad, re])
        y = np.column_stack([arr[:, 2], arr[:, 3]])  # CL, CD
        return TrainingData(x, y)
